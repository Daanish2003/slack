...
export const Profile = ({ memberId, onClose }: ProfileProps) => {
    ...
    const [UpdateDialog, confirmUpdate] = useConfrim(
        "Change role",
        "Are you sure you want to change this members role?"
    );

    const [LeaveDialog, confirmLeave] = useConfrim(
        "Leave Workspace",
        "Are you sure you want to leave this workspace?"
    );

    const [RemoveDialog, confirmRemove] = useConfrim(
        "Remove member",
        "Are you sure you want to leave this member?"
    );
    ...
    const onLeave = async () => {
        ...
        removeMember({ id: memberId }, {
            onSuccess: () => {
                router.replace("/")
                toast.success("Your left the workspace");
                onClose();
            },
            onError: () => {
        ...
        return (
            <div className="h-full flex flex-col">
                ...
                <div className="flex flex-col gap-y-2 h-full items-center justify-center">
                    ...
                    <p className="text-sn text-muted-foreground">Profile not found</p>
                </div>
            </div>
        )
    }
                ...
                <div className="flex flex-col p-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                            ...
                            <Link
                                href={`mailto:${member.user.name}`}
                                className="text-sm hover:underline text-[#1264]"
                            >
                                {member.user.email}
                            </Link>
                        </div>
